...
本仓库提供一个可扩展的统一网站，当前聚合三个频道：

- ob：组织与商业
- ur：用户与交互
- mh：心理健康

频道定义以 `config/channels.yaml` 为唯一注册入口。新增频道不得重新引入前端联合类型或发布层固定列表。
...
- 本项目不使用 `brainstorming` Skill。
- 用户给出明确的界面修改意见后，直接检查真实页面、实施并做桌面与手机验收；除非用户主动要求，不再为小范围调整增加设计规格和重复确认。
